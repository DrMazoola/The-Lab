...
This an example of sensor used to read back the volume of a specific `sink-input` using [pa_volume](blob/main/RPi_Server/PA_Volume/pa_volume.md).  The grep is used to return only the digits making up the volume in the response.  If the `sink-input` name had numeric digits a more refined regex would be needed.
```yaml
